# ACTUALIZACIÓN
Es un proceso con archivosConsiste en dos archivos (como mínimo) de entrada, denominados Maestro y Movimiento respectivamente. El maestro es el archivo que se busca actualizar en función del archivo de movimiento, cuyo código que indica que acción/es realizar por registro. Es por esto que hay como mínimo un archivo de salida (el Maestro actualizado).
...
- Baja: Elimina un registro ya existente:
    - Lógica: Se marca el registro como eliminado pero NO se lo borra del Maestro.
...
Procedimiento Modificar_Maestro() Es // Se verifica si el campo del registro de movimiento no es blanco para así modificar solamente los que se indican.
	Si (registro_moviento.campo₁ <> "") Entonces
			registro_maestro.campo₁:= registro_moviento.campo₁
		FinSi
		Si (registro_moviento.campo₂ <> "") Entonces
			registro_maestro.campo₂:= registro_moviento.campo₂
		FinSi
        Si (registro_moviento.campoₙ <> "") Entonces
			registro_maestro.campoₙ:= registro_moviento.campoₙ
		FinSi
FinProcedimiento
```
...
- In Situ: SI puedo modificar el Maestro. A su vez, NO existe un archivo de movimientos y, en su lugar, se cuenta con una terminal que indica las acciones a realizar por medio de un usuario inteligente, quien a su vez corrije los errores.
